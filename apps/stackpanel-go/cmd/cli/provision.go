...
Examples:
  stack provision                           List machines and status
  stack provision prod-server               Provision (kexec, no reformat)
  stack provision prod-server --dry-run     Print commands without running
  stack provision prod-server --reprovision Re-provision an existing machine
  stack provision prod-server --format      Provision with disko disk format`,
...
// who just want to update an existing NixOS machine should use `stack deploy`.
...
func (info *hardwareInfo) toNixConfig() string {
  ...
  boot.loader.grub = {
	...
	return fmt.Sprintf(`# Generated by stack provision (nixos-infect method).
