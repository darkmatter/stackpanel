...
#
# This file evaluates the stackpanel configuration from the flake.nix/.stack
# layout and outputs it as JSON for use by the Go CLI/agent. It eliminates state
...
let
  ...
  # @impure — only triggered when this file is evaluated without --argstr root.
  # The Go agent always passes `--argstr root`, so the env-var fallback is a
  # legacy path retained for ad-hoc `nix eval --impure -f eval.nix`.
  ...
  # ===========================================================================
  # Priority 1: Read from config JSON (Nix store path set by nix develop)
  # ===========================================================================
  configFromJson =
