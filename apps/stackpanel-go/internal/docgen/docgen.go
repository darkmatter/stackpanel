...
func generateOptionsDocs(optionsPath string, outputDir string, modulesOutputDir string) error {
	...
	if err := os.WriteFile(indexPath, []byte(escapeMDX(generateIndexMdx(categories))), 0o644); err != nil {
	...
	for category, opts := range groups {
		...
		if err := os.WriteFile(categoryPath, []byte(escapeMDX(generateCategoryMdx(category, opts))), 0o644); err != nil {
