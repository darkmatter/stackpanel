...
interface AgentEndpointContextValue {
  ...
  useLocal: (config?: LocalEndpointConfig) => void;
}

const AgentEndpointContext = createContext<AgentEndpointContextValue | null>(null);
...
function getLocalConfigFromEnv(): LocalEndpointConfig {
  ...
  const parsedPort = Number.parseInt(import.meta.env.VITE_STACKPANEL_AGENT_PORT || "", 10);
...
export function AgentEndpointProvider({ children, local }: AgentEndpointProviderProps) {
  const localEndpoint = useMemo(() => makeLocalEndpoint(local ?? getLocalConfigFromEnv()), [local]);
  ...
  const useLocal = useCallback(
    (config?: LocalEndpointConfig) => {
      ...
      setEndpoint(config ? makeLocalEndpoint(config) : localEndpoint);
  ...
  return <AgentEndpointContext.Provider value={value}>{children}</AgentEndpointContext.Provider>;
}

export function useAgentEndpoint(): AgentEndpointContextValue {
  const ctx = useContext(AgentEndpointContext);
  if (!ctx) {
    throw new Error("useAgentEndpoint must be used within <AgentEndpointProvider>");
  }
  return ctx;
}
