...
function ComboboxInput({
  ...
  showTrigger?: boolean;
  showClear?: boolean;
  inputGroupVariant?: VariantProps<typeof InputGroup>["variant"];
} & VariantProps<typeof InputGroup>) {
  return (
    <InputGroup
      className={cn("w-auto", className)}
      variant={props.inputGroupVariant}
    >
...
function ComboboxChip({
  ...
  return (
    <ComboboxPrimitive.Chip
      ...
      className={cn(
        "bg-secondary/40 text-foreground flex h-[calc(--spacing(5.5))] w-fit items-center justify-center gap-1 rounded-sm px-1.5 text-xs font-medium whitespace-nowrap has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50 has-data-[slot=combobox-chip-remove]:pr-0 border border-border/60 shadow-xs",
        className,
      )}
      {...props}
    >
