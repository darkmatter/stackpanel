...
export function ShellStatus() {
  ...
  const isCurrentlyRebuilding =
    sseRebuilding || isRebuilding || status?.rebuilding || false;
  ...
  useEffect(() => {
    if (
      autoBuild &&
      isStale &&
      !isCurrentlyRebuilding &&
      !autoBuildTriggeredRef.current
    ) {
            ...
            {(changedFiles.length > 0 || lastChangedFile) && (
              <div className="space-y-1">
                ...
                <div className="max-h-24 overflow-y-auto rounded bg-secondary/50 p-2">
                  {lastChangedFile &&
                    !changedFiles.includes(lastChangedFile) && (
