...
export function useNetwork() {
  ...
  const { data: dnsData, mutate: setDns } = useNixData<DnsData>("dns");
  const [isEnabling, setIsEnabling] = useState(false);
  const [isSavingRecord, setIsSavingRecord] = useState(false);
  const [addRecordDialogOpen, setAddRecordDialogOpen] = useState(false);
  const [viewCaConfigDialogOpen, setViewCaConfigDialogOpen] = useState(false);

  // Type-safe access to optional config properties
  const stepCaConfig = config as {
    stepCa?: {
      "step-ca"?: {
        enable?: boolean;
        "ca-url"?: string;
  ...
  const handleAddRecord = useCallback(
    async (record: NewDnsRecord) => {
      setIsSavingRecord(true);
      try {
        const currentData: DnsData = dnsData ?? { zones: {} };
        const zones = { ...currentData.zones };

        // Find the first zone key, or create a default one
        const zoneKeys = Object.keys(zones);
        const zoneKey = zoneKeys[0] ?? "default";

        const existingZone = zones[zoneKey] ?? {
          domain: dnsZoneLabel ?? "internal",
          records: [],
        };
        ...
        zones[zoneKey] = {
          ...existingZone,
          records: [...(existingZone.records ?? []), newRecord],
        };

        await setDns({ ...currentData, zones });
        setAddRecordDialogOpen(false);
      } finally {
        setIsSavingRecord(false);
      }
    },
    [dnsData, dnsZoneLabel, setDns],
  );
