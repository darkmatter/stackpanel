...
export function AgentConnect({ onConnected }: AgentConnectProps) {
  ...
  const { healthStatus, isConnected, projectRoot, pair, clearPairing, connect } =
    useAgentContext();
    ...
    return (
      <Card className="border-destructive/30 bg-destructive/5">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            The StackPanel agent is not running at {config.host}:{config.port}. Install
            the CLI and start the agent to enable local operations.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* Install CLI */}
          <div className="space-y-3">
            <p className="font-medium text-foreground text-sm">1. Install the CLI</p>
            ...
            <div className="rounded-lg border border-border bg-secondary/50 p-3 space-y-2">
              <div className="flex items-center gap-2">
                <Terminal className="h-4 w-4 text-muted-foreground" />
                <span className="text-xs text-muted-foreground">Using Homebrew</span>
              </div>
              <code className="block rounded bg-background p-2 font-mono text-muted-foreground text-xs">
          ...
          <p className="text-muted-foreground text-xs">
            No install required. The demo runs entirely in your browser against fixture
            data.
          </p>
        </CardContent>
      </Card>
  ...
  return (
    <Card className="border-yellow-500/30 bg-yellow-500/5">
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          The agent is running at {config.host}:{config.port}. Enter your pairing token
          to connect.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="rounded-lg border border-border bg-secondary/30 p-3">
                <p className="text-muted-foreground text-sm">
                  ...
                  <code className="text-foreground">stackpanel agent</code>. It looks
                  like:{" "}
