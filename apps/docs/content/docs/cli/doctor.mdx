...
Doctor never invokes a coding agent. It runs the configured file, script, network,
Nix, and acceptance checks against the current repository and runtime state.
It does not install dependencies or repair files.
...
`--setup-session` implies strict mode and requires the runtime reconciler and
...
| `--setup-session` | `string` | _none_ | Verify an expiring runtime/browser setup session; implies strict mode |
