...
are marked with `git add --intent-to-add` for Git-backed Nix evaluation. These
entries remain after success so subsequent shell entry sees the files; their
contents are not staged. A failed run removes its temporary entries. Review the
resulting repository diff. Live service availability is outside the default
`repo,build` verification scope.
