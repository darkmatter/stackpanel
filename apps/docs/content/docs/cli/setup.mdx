...
plan with configuration expectations. After acceptance, it integrates Stackpanel
into the existing flake, apps and commands. Setup keeps those expectations fixed
...
to be false. Unknown addons and choices are rejected. Remote framework sources
are pinned; explicit local sources remain local references, with an immutable
snapshot used for inspection. The resulting repository must contain `flake.lock`.
...
Only doctor determines success. A failed reconciliation or verification is sent
