...
> **Deprecated authoring surface.** Declare runtime probes under
> [`stackpanel.doctor.<module>.<name>`](/docs/reference/doctor) with `scope = "runtime"`.
> `healthchecks.modules` still works and writes into `stackpanel.doctor`; the
> `healthchecksComputed` / `healthchecksList` outputs and their `HEALTHCHECK_TYPE_*`
> wire strings are unchanged.
