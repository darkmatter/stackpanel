...
> **Deprecated.** Declare build checks under [`stackpanel.doctor.<module>.<name>`](/docs/reference/doctor)
> with `scope = "build"`. This option still works and writes into `stackpanel.doctor`;
> `moduleChecksFlattened` and `moduleChecksCertification` are unchanged read-only views.
