...
Example: stackpanel.outputs = \{ my-docs = pkgs.runCommand "docs" \{\} "..."; my-script = pkgs.writeShellScriptBin "foo" "..."; # Grouped packages (available via nix run .#scripts.\<name\>) scripts = \{ foo = pkgs.writeShellScriptBin "foo" "..."; \}; \};
