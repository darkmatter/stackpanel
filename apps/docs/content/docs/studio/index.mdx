...
<Callout type="info">
The agent starts automatically in many setups. Check `stack status` to see if it's running. If not, `stack agent` launches it in the background.
</Callout>
...
This mirrors what `stack services` does in the CLI, but with a more ergonomic interface for monitoring multiple services simultaneously.
