...
stack status              # Show service and environment status
stack services start      # Start all configured services
...
When you run `stack agent`, the binary starts a localhost HTTP server (default port 9876) that exposes 60+ API endpoints:

```bash
stack agent               # Start the agent server
```
...
The Agent wraps [process-compose](/docs/services/process-compose) to provide a unified interface for service lifecycle management. When you run `stack services start`, the Agent starts process-compose, monitors health checks, and streams status updates.
...
Common causes: syntax errors in `.stack/config.nix`, missing flake inputs, or `--impure` flag not being used.
