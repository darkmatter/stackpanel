...
<Callout type="info">
  ...
  Installer](https://github.com/DeterminateSystems/nix-installer) which enables flakes by default.
</Callout>
...
You should see a **Prelude** MOTD: project name, Getting Started commands, feature chips, and status lights. Then try the shell DX:

```bash
motd    # reprint the banner
menu    # interactive command catalogue
x       # list / dispatch commands
docs    # Markdown docs TUI (README + docs/quick-start.md)
```

See [Prelude](/docs/modules/prelude) for the full demo walkthrough and opt-out.
