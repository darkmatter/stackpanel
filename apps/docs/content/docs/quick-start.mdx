...
For users who prefer `devenv shell` over `nix develop`.
</Tab>
...
This happens during pure evaluation (like `nix flake check`). Make sure you're using the flake from within a git repository, as stackpanel uses the git root for project detection.
