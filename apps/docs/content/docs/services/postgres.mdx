...
```
.stackpanel/state/services/postgres/data/
```
...
<Callout type="warn">
The initial script only runs when the data directory is first created. If you've already started PostgreSQL, delete the data directory to re-run it: `rm -rf .stackpanel/state/services/postgres/data/`
</Callout>
...
```bash
rm -rf .stackpanel/state/services/postgres/data/
stackpanel services restart postgres
```
