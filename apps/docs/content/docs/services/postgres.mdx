...
```bash
# Start PostgreSQL
stack services start postgres

# Stop it
stack services stop postgres

# Check status
stack services status

# View logs
stack services logs postgres
...
- **Host:** `localhost`
- **Port:** Value of `$STACKPANEL_POSTGRES_PORT` (check with `stack port postgres`)
