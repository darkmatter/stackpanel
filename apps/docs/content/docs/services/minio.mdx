...
```bash
stack services stop minio
rm -rf .stack/state/minio
stack services start minio
```
...
All three services get deterministic ports and are managed together through `stack services`.
