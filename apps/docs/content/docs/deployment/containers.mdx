...
nix build .#containers.api && docker load < result
```
...
nix run .#containers.api.copyToRegistry

# Or use Docker/Podman after loading
docker load < $(nix build .#containers.api --print-out-paths)
...
```bash
nix path-info -rsSh .#containers.api
```
