...
The MOTD warning includes the fix command. You can also run it manually. See [Managing Flake Inputs](/docs/dev-environment/flake-inputs) for a full guide on the `stack flake` command.

```bash
# Add a flake input
stack flake add-input process-compose-flake github:Platonic-Systems/process-compose-flake

# Add with --no-follows to skip the nixpkgs.follows line
stack flake add-input some-input github:someone/some-input --no-follows

# Preview the changes without writing
stack flake add-input some-input github:someone/some-input --dry-run
```
...
```bash
stack flake add-input my-ext github:someone/my-ext \
