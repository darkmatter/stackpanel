...
Generated per-app/env modules export a lazy `getEnv()` singleton — they won't throw on import:

```typescript
import { web } from '@gen/env/generated';
const env = web.dev.getEnv(); // parses on first call, returns cached result after
```

You can also pass a custom env object for non-Node environments:

```typescript
const env = web.dev.getEnv({ APP_HOST: 'localhost', PORT: '3000' });
```
...
```typescript
import { loadAppEnv } from '@gen/env/loader';
...
3. Copy the data directory:
   ```dockerfile
   COPY packages/gen/env/data /app/packages/gen/env/data
   ```
...
```typescript
import { loadSharedVars } from '@gen/env/loader';
