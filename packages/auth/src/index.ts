...
if (polarClient) {
  ...
  const polarUse: Parameters<typeof polar>[0]["use"] = [
    checkout({
      ...
      successUrl: presentString(polarSuccessUrl),
  ...
  const webhookSecret = presentRedacted(polarWebhookSecret);
  if (webhookSecret) {
    polarUse.push(
      webhooks({
...
const crossSubDomainCookies =
  presentString(stackpanelDeployEnv) === "production"
...
export const auth = betterAuth({
  ...
  trustedOrigins: corsOriginValue ? [corsOriginValue] : [],
  emailAndPassword: {
    enabled: true,
  },
  advanced: {
