# Minio service configuration module
#
...
in {
  ...
  packages = [defaultPackage minioClient];
  ...
  in {
    inherit env;
    inherit dataDir configDir pidFile logFile port consolePort accessKey secretKey;
    package = minioPackage;
    client = minioClient;

    # All packages needed by the CLI to manage Minio
    allPackages = [minioPackage minioClient];
