...
let
  ...
  stackpanel-agent =
    if agentGomod2nixExists then pkgs.callPackage ../stackpanel/packages/stackpanel-agent { } else null;

  # CLI paths with optional agent
  cliPaths = [ stackpanel-cli-unwrapped ] ++ pkgs.lib.optional agentGomod2nixExists stackpanel-agent;

  # Agent PATH wrapper (only if agent exists)
  agentPathWrap = if agentGomod2nixExists then "--prefix PATH : ${stackpanel-agent}/bin" else "";
in
{
