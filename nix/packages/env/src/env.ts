...
class ServerEnv {
	...
	get STRIPE_PUBLISHABLE_KEY(): string {
		...
		if (!value)
			throw new Error("Missing required env var: STRIPE_PUBLISHABLE_KEY");
		return value;
	}
}
...
class ClientEnv {
	...
	get ANALYTICS_ID(): string | undefined {
		return (
			process.env.PUBLIC_ANALYTICS_ID ?? process.env.NEXT_PUBLIC_ANALYTICS_ID
		);
	}
	/**
	 * Stripe publishable key (safe for client)
	...
	get STRIPE_PUBLISHABLE_KEY(): string {
		...
		if (!value)
			throw new Error(
				"Missing required env var: PUBLIC_STRIPE_PUBLISHABLE_KEY",
			);
		return value;
	}
}
