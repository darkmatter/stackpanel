...
# ==============================================================================
{
  ...
  getPort = {
    ...
    if explicit != null
    then explicit
    else if builtins.hasAttr key ports && (ports.${key} ? port)
    then ports.${key}.port
    else fallbackPorts.${key};
in {
  ...
  mkGlobalServices = {
    ...
    postgresPort = getPort {key = "POSTGRES"; explicit = cfg.postgres.port; inherit ports;};
    redisPort = getPort {key = "REDIS"; explicit = cfg.redis.port; inherit ports;};
    minioPort = getPort {key = "MINIO"; explicit = cfg.minio.port; inherit ports;};
    minioConsolePort = getPort {key = "MINIO_CONSOLE"; explicit = cfg.minio.consolePort; inherit ports;};

    postgresService = lib.optionalAttrs cfg.postgres.enable (servicesLib.mkGlobalPostgres {
    ...
    redisService = lib.optionalAttrs cfg.redis.enable (servicesLib.mkGlobalRedis {
      projectName = cfg.projectName;
      port = redisPort;
      package = cfg.redis.package;
    });

    minioService = lib.optionalAttrs cfg.minio.enable (servicesLib.mkGlobalMinio {
      projectName = cfg.projectName;
      port = minioPort;
