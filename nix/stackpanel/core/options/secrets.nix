...
let
  ...
  secretsGroupModule = { name, config, ... }: {
  ...
  initializedGroups = lib.filterAttrs
    (_: g: let pub = g.age-pub or ""; in pub != null && pub != "")
...
in
{
  ...
  config = lib.mkIf config.stackpanel.secrets.enable {
    ...
    stackpanel.secrets.master-keys = lib.mkDefault (
      { local = defaultLocalKey; } // groupMasterKeys
    );
