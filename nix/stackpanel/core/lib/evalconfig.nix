...
#   nix eval --json -f evalconfig.nix --argstr root /path/to/project
#
# Legacy usage (impure — reads env vars):
#   nix eval --impure --json -f evalconfig.nix
#
...
else
  {
    ...
    hint =
      if root == null then
        ...
        "No config at ${effectiveRoot}/.stack/state/stackpanel.json — run 'stack preflight' first";
