...
# core.nix
#
# Core implementation of the secrets module scripts for devenv context.
# Provides the actual script implementations for secrets management.
#
# Scripts provided:
# - ensure-age-key-dev: Validates that the required AGE key exists locally
# - sops: Wrapped SOPS command with automatic key preflight check
# - generate-secrets-schema: Generates typed code from SOPS-encrypted YAML
# - generate-secrets-package: Orchestrates code generation for all environments
#
# Note: This module includes test configuration when running `devenv test`.
# ==============================================================================
{pkgs, lib, config, ... }: let
