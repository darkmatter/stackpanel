...
let
  ...
  mkFlyToml =
    ...
    ''
      ...
      #   4. Deploy:           flyctl deploy --config ${appPath}/fly.toml --image registry.fly.io/${flyAppName}:latest
      #
  ...
  mkDeployPackageScripts =
    ...
    lib.foldlAttrs (
      ...
      // {
        ...
        "deploy:${appName}" =
          "cd ../.. && (flyctl status -a ${flyAppName} > /dev/null 2>&1 || flyctl apps create ${flyAppName} ${orgFlag}) && flyctl deploy --config ${appPath}/fly.toml --image registry.fly.io/${flyAppName}:latest";
        # Full workflow
  ...
  mkAppDeployScriptDerivations =
    ...
    let
      flyAppName = appCfg.deployment.fly.appName or appName;
      appPath = appCfg.path or "apps/${appName}";
      flyConfigPath = "${appPath}/fly.toml";
    in
    {
      # Note: container-build and container-push scripts are now generated by
      # the containers module for ALL apps with containers enabled (not just Fly).
