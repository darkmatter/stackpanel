  ...
  messages = {
    Task = proto.mkMessage {
      ...
      fields = {
        ...
        exec = proto.optional (
          proto.withExample "bun run build" (
            proto.string 1 "Shell script to execute (compiled to Nix derivation)"
          )
        );
        description = proto.optional (
          proto.withExample "Build all packages" (proto.string 2 "Human-readable description of the task")
        );
        cwd = proto.optional (
          proto.withExample "apps/web" (
        ...
        depends_on = proto.repeated (
          proto.withExample "^build" (proto.string 5 "Tasks that must complete first (use ^ for deps)")
        );
        outputs = proto.repeated (
          proto.withExample "dist/**" (proto.string 6 "Output file globs for caching (e.g. dist/**)")
        );
        inputs = proto.repeated (
          proto.withExample "$TURBO_DEFAULT$" (
            proto.string 7 "Input file globs for cache key (e.g. $TURBO_DEFAULT$)"
          )
        );
        persistent = proto.optional (
          proto.withExample false (proto.bool 8 "Long-running process (e.g. dev server)")
        );
