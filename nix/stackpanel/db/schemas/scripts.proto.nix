...
proto.mkProtoFile {
  ...
  messages = {
    ...
    ScriptArg = proto.mkMessage {
      ...
      '';
      fields = {
        name = proto.withExample "--dry-run" (
          proto.string 1 "Argument name (e.g., 'file', '--output', '-v')"
        );
        description = proto.optional (
          proto.withExample "Preview changes without applying" (
            proto.string 2 "Human-readable description of the argument"
          )
        );
        required = proto.optional (
          proto.withExample false (proto.bool 3 "Whether the argument is required (default: false)")
        );
        default = proto.optional (
          proto.withExample "production" (proto.string 4 "Default value if not provided")
        );
      };
    };
    ...
    Script = proto.mkMessage {
      ...
      fields = {
        ...
        exec = proto.optional (
          proto.withExample "bun run dev" (
            proto.string 1 "Shell command to execute (mutually exclusive with path)"
          )
        );
        description = proto.optional (
        ...
        bin_path = proto.optional (
          proto.withExample "/nix/store/abc123-scripts/bin/db-seed" (
            proto.string 4 "Path to script executable in Nix store (computed)"
          )
        );
        source = proto.optional (
          proto.withExample "inline" (proto.string 5 "Source type: inline or path (for debugging)")
        );
