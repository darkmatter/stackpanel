  ...
  messages = {
    ...
    Healthcheck = proto.mkMessage {
      ...
      fields = {
        ...
        description = proto.optional (
          proto.withExample "Verifies the PostgreSQL service is accepting connections on its assigned port" (
            proto.string 3 "Description of what this check verifies"
          )
        );
        ...
        script_bin_path = proto.optional (
          proto.withExample "/nix/store/abc123-pg-check/bin/pg-check" (
            proto.string 6 "Path to script executable in Nix store"
          )
        );
        script_source = proto.optional (
          proto.withExample "scriptRef" (proto.string 7 "Source type: inline, path, scriptRef, package")
        );

        # Nix-based checks
        nix_expr = proto.optional (
          proto.withExample "config.services.postgres.enable" (
            proto.string 8 "Nix expression to evaluate (for NIX type)"
          )
        );

        # HTTP-based checks
        http_url = proto.optional (
          proto.withExample "http://localhost:6402/health" (proto.string 9 "URL to check (for HTTP type)")
        );
        http_method = proto.optional (
          proto.withExample "GET" (proto.string 10 "HTTP method (GET, POST, etc.)")
        );
        http_expected_status = proto.optional (
          proto.withExample 200 (proto.int32 11 "Expected HTTP status code")
        );

        # TCP-based checks
        tcp_host = proto.optional (
          proto.withExample "localhost" (proto.string 12 "Host to connect to (for TCP type)")
        );
        tcp_port = proto.optional (
          proto.withExample 6410 (proto.int32 13 "Port to connect to (for TCP type)")
        );

        # Timing
        timeout_seconds = proto.withExample 5 (proto.int32 14 "Timeout for the check in seconds");
        interval_seconds = proto.optional (
          proto.withExample 30 (proto.int32 15 "How often to run this check (optional)")
        );

        # Metadata
        module = proto.withExample "postgres" (proto.string 16 "Module that registered this healthcheck");
        tags = proto.repeated (
          proto.withExample "service" (proto.string 17 "Tags for filtering/grouping checks")
        );
      };
    };
    ...
    HealthcheckResult = proto.mkMessage {
      ...
      fields = {
        ...
        message = proto.optional (
          proto.withExample "PostgreSQL responded in 12ms" (proto.string 3 "Human-readable result message")
        );
        error = proto.optional (
          proto.withExample "connection refused" (proto.string 4 "Error message if check failed to execute")
        );
        output = proto.optional (
          proto.withExample "psql: connected to localhost:6410" (
            proto.string 5 "Raw output from script/command"
          )
        );
        duration_ms = proto.withExample 12 (proto.int64 6 "How long the check took to run in milliseconds");
        timestamp = proto.withExample "2026-04-30T18:21:04Z" (
          proto.string 7 "When the check was run (RFC3339)"
        );
      };
    };
    ...
    ModuleHealth = proto.mkMessage {
      ...
      fields = {
        ...
        last_updated = proto.withExample "2026-04-30T18:21:04Z" (
          proto.string 6 "When health was last evaluated (RFC3339)"
        );
      };
    };
    ...
    HealthSummary = proto.mkMessage {
      ...
      fields = {
        ...
        last_updated = proto.withExample "2026-04-30T18:21:04Z" (
          proto.string 5 "When summary was last computed (RFC3339)"
        );
      };
    };
  };
