  ...
  messages = {
    ...
    AppDeploy = proto.mkMessage {
      ...
      fields = {
        ...
        targets = proto.repeated (
          proto.withExample "prod-web-01" (proto.string 2 "Target machine ids or tag selectors")
        );
        role = proto.optional (
          proto.withExample "web" (proto.string 3 "Deployment role label for this app")
        );
        nixos_modules = proto.repeated (
          proto.withExample "./modules/nginx.nix" (
            proto.string 4 "Extra NixOS modules to import for this app"
          )
        );
        system = proto.optional (
          proto.withExample "x86_64-linux" (proto.string 5 "Target system/architecture (e.g., x86_64-linux)")
        );
        secrets = proto.repeated (
          proto.withExample "DATABASE_URL" (
            proto.string 6 "Secret references required by this app during deploy"
          )
        );
      };
    };
    ...
    AppEnvironment = proto.mkMessage {
      ...
      fields = {
        ...
        description = proto.optional (
          proto.withExample "Local development environment" (
            proto.string 2 "(optional) Description of the environment"
          )
        );
    ...
    EnvironmentVariable = proto.mkMessage {
      ...
      fields = {
        key = proto.withExample "DATABASE_URL" (
          proto.string 1 "ID of the environment variable - defaults to key used in the attribute path. KEY will be read from $KEY in the environment"
        );
        ...
        value = proto.optional (
          proto.withExample "postgres://localhost:5432/app" (
            proto.string 4 "Value of the environment variable"
          )
        );
        sops = proto.optional (
          proto.withExample ".stack/secrets/dev.yaml" (
            proto.string 5 "Path to the SOPS file for this variable's group"
          )
        );
        defaultValue = proto.optional (
          proto.withExample "postgres://localhost:5432/app" (
            proto.string 6 "Default value of the environment variable"
          )
        );
        description = proto.optional (
          proto.withExample "Postgres connection string used by the API server" (
            proto.string 7 ''
