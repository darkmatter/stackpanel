...
#
# Provides credential helper scripts that use device certificates (from Step CA)
# to obtain temporary AWS credentials via IAM Roles Anywhere. Includes caching
# to minimize API calls while respecting credential expiration.
#
# Features:
#   - aws-creds-env: Exports AWS credentials as environment variables
#   - Automatic credential caching with configurable buffer time
#   - Pre-flight certificate validation
#   - Integration with Step CA device certificates
#
# Usage:
