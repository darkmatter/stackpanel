...
let
  ...
  toStorePath =
    name: input:
    if builtins.isString input && lib.hasPrefix "-----BEGIN" input then
      pkgs.writeText name input
    else
      input;
in
{
  ...
  mkAwsCredentialProcess =
    ...
    let
      ...
      intermediatesFile =
        if intermediates != null then toStorePath "aws-intermediates.pem" intermediates else null;
      roleArn = "arn:aws:iam::${accountId}:role/${roleName}";
    in
    pkgs.writeShellScriptBin "aws-credential-process" ''
      set -euo pipefail
