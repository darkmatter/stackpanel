...
#         generated-payloads/registry.ts — payload registry
...
let
  ...
  payloadRegistryModule =
    let
      ...
      appBlocks = lib.concatMapStringsSep "\n" appBlock appNames;
    in
    "// Auto-generated by Stackpanel — do not edit manually.\n"
  ...
  # ---------------------------------------------------------------------------
  # Effect-native per-app module — `src/effect/<app>.ts`.
  #
  # Generates an `effect/Schema` class describing the env shape (with
  # `Schema.RedactedFromValue` wrapping any `secret` / SOPS-backed field), an
  # `effect/Context` Service tag keyed by the env shape, and one `Layer`
  # per environment ID. Each layer:
  #
  #   1. Calls `loadEnvEffect(app, env, { validate: true, inject: false })`,
  #      which decrypts the SOPS payload and validates `required` vars.
  #   2. Decodes the resulting payload through the schema, lifting plain
  #      strings into `Redacted<string>` for secret fields.
  #   3. Provides the decoded class instance under the Service tag.
  #
  # Callers consume it with the standard Effect Service pattern:
  #
  ...
  # Field type expression for the generated `Schema.Class` body.
  #   - secret / SOPS-backed → `Schema.RedactedFromValue(Schema.String)`
  #   - everything else      → `Schema.String`
  #   - not loader-guaranteed → wrapped in `Schema.optional(...)`
  schemaFieldExpr =
    meta:
    let
      base =
        if meta.secret || meta.sops != null then
          "Schema.RedactedFromValue(Schema.String)"
        else
          "Schema.String";
    in
    if isLoaderGuaranteed meta then base else "Schema.optional(${base})";
  ...
  mkAppEffectExport =
    ...
    let
      appEnv = getAppEnv apps.${appName};
      sortedKeys = lib.sort (a: b: a < b) (lib.attrNames appEnv);
      schemaClass = "${pascalCase appName}EnvSchema";
      envTypeAlias = "${pascalCase appName}Env";
      serviceTag = "${pascalCase appName}EnvTag";
      serviceKey = "@gen/env/${appName}";

      fieldLines = lib.concatMapStringsSep "\n" (
        key: "  ${key}: ${schemaFieldExpr appEnv.${key}},"
      ) sortedKeys;

      envs = availableAppEnvs.${appName} or [ ];
      sortedEnvs = lib.sort (a: b: a < b) envs;
      layerLines = lib.concatMapStringsSep "\n" (
        env: "export const ${env} = layer(${builtins.toJSON env});"
      ) sortedEnvs;
      layerBlock = if sortedEnvs == [ ] then "" else "\n\n${layerLines}";
    in
    ''
      // Auto-generated by Stackpanel — do not edit manually.
      // Effect-native env accessor for the `${appName}` app — derived from
      // `apps.${appName}.env`.
      //
      //   import { ${serviceTag}, dev as ${pascalCase appName}EnvDev } from "${packageName}/effect/${appName}";
      //
      //   const program = Effect.gen(function* () {
      //     const env = yield* ${serviceTag};
      //     return env.PORT;
      //   }).pipe(Effect.provide(${pascalCase appName}EnvDev));
      ...
      const APP = ${builtins.toJSON appName};

      export class ${schemaClass} extends Schema.Class<${schemaClass}>(${builtins.toJSON schemaClass})({
      ${fieldLines}
      }) {}

      export type ${envTypeAlias} = ${schemaClass};

      export class ${serviceTag} extends Context.Service<${serviceTag}, ${envTypeAlias}>()(${builtins.toJSON serviceKey}) {}

      const decode = Schema.decodeUnknownEffect(${schemaClass});
  ...
  # ---------------------------------------------------------------------------
  # Effect-native per-scope module — `src/effect/scope/<scope>.ts`.
  #
  # Same shape as `mkAppEffectExport` but for bare cross-cutting root scopes
  # (entries in `stackpanel.envs.<scope>` keyed by a name like `deploy`).
  # The single emitted `Layer` is named `layer` and resolves the SOPS payload
  # via `loadEnvScopeEffect(scope, ...)`.
  # ---------------------------------------------------------------------------
  mkScopeEffectExport =
    scopeName:
    let
      scopeVars = lib.mapAttrs normalizeEnvVar (rootEnvs.${scopeName} or { });
      sortedKeys = lib.sort (a: b: a < b) (lib.attrNames scopeVars);
      schemaClass = "${pascalCase scopeName}ScopeSchema";
      scopeTypeAlias = "${pascalCase scopeName}Scope";
      serviceTag = "${pascalCase scopeName}ScopeTag";
      serviceKey = "@gen/env/_envs/${scopeName}";

      fieldLines = lib.concatMapStringsSep "\n" (
        key: "  ${key}: ${schemaFieldExpr scopeVars.${key}},"
      ) sortedKeys;
    in
    ''
      // Auto-generated by Stackpanel — do not edit manually.
      // Effect-native env accessor for the `${scopeName}` root env scope —
      // derived from `stackpanel.envs.${scopeName}`.
      //
      //   import { ${serviceTag}, layer as ${pascalCase scopeName}ScopeLive } from "${packageName}/effect/scope/${scopeName}";
      //
      //   const program = Effect.gen(function* () {
      //     const scope = yield* ${serviceTag};
      //     return scope.SOME_KEY;
      //   }).pipe(Effect.provide(${pascalCase scopeName}ScopeLive));
      ...
      const SCOPE = ${builtins.toJSON scopeName};

      export class ${schemaClass} extends Schema.Class<${schemaClass}>(${builtins.toJSON schemaClass})({
      ${fieldLines}
      }) {}

      export type ${scopeTypeAlias} = ${schemaClass};

      export class ${serviceTag} extends Context.Service<${serviceTag}, ${scopeTypeAlias}>()(${builtins.toJSON serviceKey}) {}
  ...
  # Scopes that should get an Effect module. Same filter as the SOPS payload
  ...
  mkRootIndexModule =
    let
      sortedApps = lib.sort (a: b: a < b) appsWithEnv;
      sortedScopes = lib.sort (a: b: a < b) scopesWithEffectExport;
    ...
    ''
      ...
      import { loadEnv${scopeImport}, type LoadEnvOptions } from "./runtime/node-loader";

      ${appBlocks}${scopeSection}

      ${loadersBlock}
    '';

  # ---------------------------------------------------------------------------
  # Effect-native barrel — `src/effect/index.ts`.
  #
  # Aggregates the per-app and per-scope Effect modules into a single
  # `effectLoaders` object that mirrors the shape of `loaders` in the root
  # barrel:
  #
  #   import { effectLoaders } from "@gen/env/effect";
  #
  #   const program = base.pipe(Effect.provide(effectLoaders.web.dev));
  #   const deploy  = other.pipe(Effect.provide(effectLoaders.deploy));
  #
  # For apps each property is a `Layer` (one per env). For scopes the value
  # is the scope's single `Layer`.
  # ---------------------------------------------------------------------------
  mkEffectIndexModule =
    let
      sortedApps = lib.sort (a: b: a < b) appsWithEnv;
      ...
      mkScopeImport =
        scope:
        let
          alias = "${toJsIdentifier scope}Scope";
        in
        "import { layer as ${alias} } from ${builtins.toJSON "./scope/${scope}"};";
      scopeImports = map mkScopeImport sortedScopes;

      allImports = lib.concatStringsSep "\n" (appImports ++ scopeImports);
      ...
      mkAppEntry =
        ...
        in
        "  ${jsName}: {${body}},";
      appEntries = lib.concatMapStringsSep "\n" mkAppEntry sortedApps;
    ...
    ''
      ...
      // root barrel:
      //
      //   import { effectLoaders } from "${packageName}/effect";
      //
      ...
      export const effectLoaders = {${entries}} as const;
    '';
  ...
  generatedFiles =
    let
      ...
      perAppEffectExports = lib.listToAttrs (
        map (appName: {
          name = "${generatedDir}/effect/${appName}.ts";
          value = {
            kind = "text";
            content = mkAppEffectExport appName;
          };
        }) appsWithEnv
      );

      perScopeEffectExports = lib.listToAttrs (
        map (scope: {
          name = "${generatedDir}/effect/scope/${scope}.ts";
          value = {
            kind = "text";
            content = mkScopeEffectExport scope;
          };
        }) scopesWithEffectExport
      );
