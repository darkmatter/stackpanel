...
# Provides helpers for VS Code terminal integration, workspace files, and
...
# ==============================================================================
{
  ...
  mkDevshellLoader = {
    ...
    execCommand =
      if exec != null
      then exec
      else if shellMode == "stackpanel"
      then ''        DEV_SCRIPT="$ROOT/devshell"
      ...
      ''
      else if shellMode == "flake"
      then ". <(nix print-dev-env --impure)"
