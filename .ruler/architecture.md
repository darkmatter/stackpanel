...
meta.nix (declares flakeInputs)
  -> module.nix (wires to stack.modules.*.flakeInputs via `flakeInputs = meta.flakeInputs or []`)
...
| File | Role |
|---|---|
| `nix/stack/modules/*/meta.nix` | Declares `flakeInputs` (pure data) |
| `nix/stack/modules/*/module.nix` | Wires `meta.flakeInputs or []` into module registration |
| `nix/stack/core/options/modules.nix` | Defines `flakeInputs` option type, serializes in `computeSerializableModule` |
| `nix/stack/core/cli.nix` | Receives `inputs` via specialArgs, computes `missingFlakeInputs` by filtering enabled modules' declared inputs against available flake inputs |
| `apps/stack-go/internal/nixconfig/nixconfig.go` | `MissingFlakeInput` struct, parsed from state file |
| `apps/stack-go/internal/tui/motd_data.go` | `CollectIssues` generates warnings for missing inputs |
| `apps/stack-go/cmd/cli/motd.go` | Passes config's missing inputs into MOTD data |

**CLI auto-install** (`apps/stack-go/internal/flakeedit/`):
The `flakeedit` package uses tree-sitter-nix to surgically edit `flake.nix` -- adding inputs and `stackpanelImports` entries without disturbing comments or formatting. The CLI command `stack flake add-input <name> <url>` writes the input, optionally adds a `stackpanelImports` entry, and runs `nix flake lock`. The agent's `handleRegistryInstall` endpoint uses `flakeedit` to auto-install inputs when modules are installed from the registry.
...
Key directories:
- `.stack/secrets/.sops.yaml` -- Generated from `stack.secrets.recipients` and tag-based group selection
...
Team onboarding: add a public key in `stack.secrets.recipients` or `stack.users`, re-enter the devshell to regenerate `.sops.yaml`, then run `.stack/secrets/bin/rekey.sh` so existing secret files pick up the new recipients.
...
When modifying any part of the system, you must propagate changes to all affected layers. Stack is a full-stack system where Nix modules, Go CLI, TypeScript packages, codegen, templates, UI, and docs are tightly coupled. A change in one layer almost always requires updates in others.
