...
// ----------------------------------------------------------------------------
const USE_DEVENV =
  process.env.IN_NIX_SHELL === "impure" || process.env.DEVENV_ROOT !== undefined;
...
// ----------------------------------------------------------------------------
function computeProjectPort(name: string, minPort = 3000, portRange = 7000): number {
...
export interface DevenvPostgres extends DevenvPostgresProps {
  /** Connection URL */
  connectionUrl: string;
}

export const DevenvPostgres = Resource(
  "devenv::Postgres",
  async function (
...
// ----------------------------------------------------------------------------
export interface DevenvRedisProps {
...
export interface DevenvRedis extends DevenvRedisProps {
  /** Connection URL */
  connectionUrl: string;
}

export const DevenvRedis = Resource(
  "devenv::Redis",
  async function (
...
// ----------------------------------------------------------------------------
export interface DevenvCaddyProps {
...
export interface DevenvCaddy extends DevenvCaddyProps {
...
export const server = await cloudflare.Worker("server", {
  ...
  bindings: {
    ...
    REDIS_URL: redisUrl,
    ...(redisToken ? { UPSTASH_REDIS_REST_TOKEN: alchemy.secret(redisToken) } : {}),
